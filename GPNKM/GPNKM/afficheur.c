...
void scoreMonitor(int queue_id, TmsgbufAdr adr_msg, int type, int sem_control, int sem_DispSrv)
{
	// INIT SECTION
...
void showTRMenu(int queue_id, TmsgbufAdr adr_msg, int sem_type, int sem_control, int sem_DispSrv)
{
	fflush(stdout);
//	system ( "clear" );
	printf("\033[36m");
	...
	switch(getchar())
	{
		case '1' : show_notice("Monitor", "Trial 1 is going to begin");
				   ...
				   scoreMonitor(queue_id, adr_msg, SIGTR1, sem_control, sem_DispSrv);
				   break;
		case '2' : show_notice("Monitor", "Trial 2 is going to begin");
				   sendSig(SIGTR2, sem_type, 0);
				   scoreMonitor(queue_id, adr_msg, SIGTR2, sem_control, sem_DispSrv);
				   break;
		case '3' : show_notice("Monitor", "Trial 3 is going to begin");
				   sendSig(SIGTR3, sem_type, 0);
				   scoreMonitor(queue_id, adr_msg, SIGTR3, sem_control, sem_DispSrv);
				   break;
		case '4' : printf("Show Results\n"); break;
		case '0' : showMainMenu(queue_id, adr_msg, sem_type, sem_control, sem_DispSrv); break;
		default  : showTRMenu(queue_id, adr_msg, sem_type, sem_control, sem_DispSrv); break;
	}
	fflush(stdin);
}

void showQualifMenu(int queue_id, TmsgbufAdr adr_msg,  int sem_type, int sem_control, int sem_DispSrv)
{
	fflush(stdout);
	system ( "clear" );
	printf("\033[36m");
	...
	switch(getchar())
	{
		case '1' : show_notice("Monitor", "Qualification 1 is going to begin");
				   ...
				   scoreMonitor(queue_id, adr_msg, SIGQU1, sem_control, sem_DispSrv);
				   break;
		case '2' : show_notice("Monitor", "Qualification 1 is going to begin");
				   sendSig(SIGQU2, sem_type, 0);
				   scoreMonitor(queue_id, adr_msg, SIGQU2, sem_control, sem_DispSrv);
				   break;;
		case '3' : show_notice("Monitor", "Qualification 1 is going to begin");
				   sendSig(SIGQU3, sem_type, 0);
				   scoreMonitor(queue_id, adr_msg, SIGQU3, sem_control, sem_DispSrv);
				   break;
		case '4' : printf("Show Results of Qualifiers\n"); break;
		case '0' : showMainMenu(queue_id, adr_msg, sem_type, sem_control, sem_DispSrv); break;
		default  : showQualifMenu(queue_id, adr_msg, sem_type, sem_control, sem_DispSrv); break;
	}   
	fflush(stdin);
}

void endOfProgram(int queue_id, TmsgbufAdr adr_msg,  int sem_type, int sem_control, int sem_DispSrv)
{
...
void showMainMenu(int queue_id, TmsgbufAdr adr_msg, int sem_type, int sem_control, int sem_DispSrv)
{
	...
	switch(getchar())
	{
		case '1' : showTRMenu(queue_id, adr_msg, sem_type, sem_control, sem_DispSrv); break;
		case '2' : showQualifMenu(queue_id, adr_msg, sem_type, sem_control, sem_DispSrv); break;
		case '3' : show_notice("Monitor", "Grand Prix is going to begin");
				   sendSig(SIGGP, sem_type, 0);
				   scoreMonitor(queue_id, adr_msg, SIGGP, sem_control, sem_DispSrv);
				   break;
		case '4' : printf("Show Results\n"); break;
		case '5' : printf("Restart Grand Prix\n"); break;
		case '0' : endOfProgram(queue_id, adr_msg, sem_type, sem_control, sem_DispSrv);
		default  : showMainMenu(queue_id, adr_msg, sem_type, sem_control, sem_DispSrv); break;
	}
    fflush(stdin);
}
